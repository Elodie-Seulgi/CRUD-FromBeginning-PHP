...
class PosteForm extends Form
{
    public function __construct(string $action = "#")
    {
        $this->startForm($action, 'POST', ['class' => 'form card p-3 w-75 mx-auto'])
            ...
            ->addInput('text', 'title', ['class' => 'form-control', 'id' => 'title', 'required' => true])
            ->endDiv()
            ->startDiv(['class' => 'mb-3'])
            ->addLabel('description', 'Description', ['class' => 'form-label'])
            ->addTextarea('description', '', ['class' => 'form-control', 'id' => 'description', 'rows' => 5, 'placeholder' => 'Description du poste', 'required' => true])
            ->endDiv()
            ->startDiv(['class' => 'mb-3 form-check form-switch'])
            ->addInput('checkbox', 'enabled', ['class' => 'form-check-input', 'id' => 'enabled'])
